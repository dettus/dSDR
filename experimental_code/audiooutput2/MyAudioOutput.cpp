...
MyAudioOutput::MyAudioOutput(QObject *parent) :
	QIODevice(parent),
	m_initialized(false),
	m_output(0),
	m_volume(0.0f)
...
bool MyAudioOutput::init(int bits, int channels, int samplerate)
{
	if (m_initialized) {
		return false;
	}
...
bool MyAudioOutput::setDevice(QAudioDeviceInfo deviceInfo)
{
...
void MyAudioOutput::setVolume(float volume)
{
	m_volume = volume;
}

void MyAudioOutput::output(const QByteArray & data)
{
	if (m_output && m_output->state() != QAudio::StoppedState) {
...
qint64 MyAudioOutput::readData(char *data, qint64 maxlen)
{
...
qint64 MyAudioOutput::writeData(const char *data, qint64 len)
{
	Q_UNUSED(data);
	Q_UNUSED(len);
...
qint64 MyAudioOutput::bytesAvailable() const
{
	return m_buffer.length() + QIODevice::bytesAvailable();
}

bool MyAudioOutput::isSequential() const
{
	return true;
}

void MyAudioOutput::notified()
{
}

void MyAudioOutput::stateChanged(QAudio::State state)
{
